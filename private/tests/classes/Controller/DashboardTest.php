...
class Controller_DashboardTest extends TestCase
{
    ...
    public function testActionProject()
    {
        ...
        $expectedWidgets = array(
            Request::factory('w/project/Background/display/' . $this->genNumbers['projectFooBackground'] . '/' . $this->genNumbers['ProjectFoo'])->execute(),
            Request::factory('w/project/Log/display/' . $this->genNumbers['projectFooLog'] . '/' . $this->genNumbers['ProjectFoo'])->execute(),
        );
        $expected        = View::factory('dashboard')
                ->set('from', 'project')
    ...
    public function testActionBuild()
    {
        ...
        $expectedWidgets = array(
            Request::factory('w/build/Background/display/' . $this->genNumbers['buildFooBackground'] . '/' . $this->genNumbers['build1'])->execute(),
            Request::factory('w/build/Log/display/' . $this->genNumbers['buildFooLog'] . '/' . $this->genNumbers['build1'])->execute(),
        );
        $expected        = View::factory('dashboard')
                ->set('from', 'build')
                ->set('projectId', $this->genNumbers['ProjectFoo'])
